<?php
require_once __DIR__ . '/../includes/bootstrap.php';
require_role('admin');

$id = (int)($_GET['id'] ?? $_POST['id'] ?? 0);
if (!$id) { flash_set('error', 'Produit introuvable.'); redirect('produits.php'); }

$cats     = $pdo->query("SELECT id, nom FROM categorie ORDER BY nom")->fetchAll();
$artisans = $pdo->query("SELECT id, nom_boutique FROM artisan ORDER BY nom_boutique")->fetchAll();
$errors   = [];

$st = $pdo->prepare(
    "SELECT p.*, ip.url AS image_url
       FROM produit p
       LEFT JOIN image_produit ip ON ip.produit_id = p.id AND ip.est_principale = 1
      WHERE p.id = ?"
);
$st->execute([$id]);
$p = $st->fetch();
if (!$p) { flash_set('error', 'Produit introuvable.'); redirect('produits.php'); }

if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    csrf_check();
    $nom   = trim($_POST['nom'] ?? '');
    $prix  = $_POST['prix'] ?? '';
    $cat   = (int)($_POST['categorie_id'] ?? 0);
    $artid = (int)($_POST['artisan_id'] ?? 0);
    if (!$nom)  $errors[] = "Nom requis.";
    if (!is_numeric($prix) || (float)$prix <= 0) $errors[] = "Prix invalide.";
    if (!$cat)  $errors[] = "Catégorie requise.";
    if (!$artid) $errors[] = "Artisan requis.";

    if (!$errors) {
        $pdo->prepare(
            "UPDATE produit SET artisan_id=?, categorie_id=?, nom=?, description=?, prix=?,
                                materiaux=?, dimensions=?, delai_fabrication_jours=?,
                                stock=?, est_personnalisable=?, est_publie=?
             WHERE id=?"
        )->execute([
            $artid, $cat, $nom,
            trim($_POST['description'] ?? '') ?: null,
            (float)$prix,
            trim($_POST['materiaux'] ?? '') ?: null,
            trim($_POST['dimensions'] ?? '') ?: null,
            $_POST['delai_fabrication_jours'] !== '' ? (int)$_POST['delai_fabrication_jours'] : null,
            (int)($_POST['stock'] ?? 0),
            isset($_POST['est_personnalisable']) ? 1 : 0,
            isset($_POST['est_publie']) ? 1 : 0,
            $id,
        ]);

        // Photo : upload ou URL
        $image_url = trim($_POST['image_url'] ?? '');
        $uploaded  = '';
        if (!empty($_FILES['photo']['name']) && $_FILES['photo']['error'] === UPLOAD_ERR_OK) {
            $allowed = ['image/jpeg'=>'jpg','image/png'=>'png','image/webp'=>'webp'];
            $mime    = mime_content_type($_FILES['photo']['tmp_name']);
            if (isset($allowed[$mime]) && $_FILES['photo']['size'] <= 5 * 1024 * 1024) {
                $dir = __DIR__ . '/../uploads/produits';
                if (!is_dir($dir)) @mkdir($dir, 0755, true);
                $fname = "p{$id}-" . bin2hex(random_bytes(4)) . '.' . $allowed[$mime];
                if (move_uploaded_file($_FILES['photo']['tmp_name'], "$dir/$fname")) {
                    $uploaded = "uploads/produits/$fname";
                }
            }
        }
        $final_url = $uploaded ?: $image_url;
        if ($final_url) {
            $pdo->prepare("UPDATE image_produit SET est_principale=0 WHERE produit_id=?")->execute([$id]);
            $pdo->prepare("INSERT INTO image_produit (produit_id, url, ordre, est_principale) VALUES (?,?,0,1)")
                ->execute([$id, $final_url]);
        }

        flash_set('success', 'Produit mis à jour.');
        redirect('produits.php');
    }
    $p = array_merge($p, $_POST);
}

$page_title = 'Modifier produit — Backoffice';
require_once __DIR__ . '/_header.php';
?>
<div class="crumb"><a href="index.php">Backoffice</a> &rsaquo; <a href="produits.php">Produits</a> &rsaquo; Modifier</div>
<div class="form-card" style="max-width:680px">
  <h1>Modifier le produit #<?= $id ?></h1>
  <?php foreach ($errors as $e): ?><div class="flash flash-error"><?= h($e) ?></div><?php endforeach; ?>
  <form method="post" action="produit_edit.php" enctype="multipart/form-data">
    <?= csrf_field() ?>
    <input type="hidden" name="id" value="<?= $id ?>">

    <div class="form-row-group">
      <div class="form-row"><label>Nom *</label>
        <input type="text" name="nom" required value="<?= h($p['nom']) ?>"></div>
      <div class="form-row"><label>Prix (€) *</label>
        <input type="number" min="0" step="0.01" name="prix" required value="<?= h($p['prix']) ?>"></div>
    </div>

    <div class="form-row-group">
      <div class="form-row"><label>Artisan *</label>
        <select name="artisan_id" required>
          <?php foreach ($artisans as $a): ?>
            <option value="<?= (int)$a['id'] ?>" <?= (int)$p['artisan_id']===(int)$a['id']?'selected':'' ?>><?= h($a['nom_boutique']) ?></option>
          <?php endforeach; ?>
        </select></div>
      <div class="form-row"><label>Catégorie *</label>
        <select name="categorie_id" required>
          <?php foreach ($cats as $c): ?>
            <option value="<?= (int)$c['id'] ?>" <?= (int)$p['categorie_id']===(int)$c['id']?'selected':'' ?>><?= h($c['nom']) ?></option>
          <?php endforeach; ?>
        </select></div>
    </div>

    <div class="form-row"><label>Description</label>
      <textarea name="description"><?= h($p['description'] ?? '') ?></textarea></div>

    <div class="form-row-group">
      <div class="form-row"><label>Matériaux</label>
        <input type="text" name="materiaux" value="<?= h($p['materiaux'] ?? '') ?>"></div>
      <div class="form-row"><label>Dimensions</label>
        <input type="text" name="dimensions" value="<?= h($p['dimensions'] ?? '') ?>"></div>
    </div>

    <div class="form-row-group">
      <div class="form-row"><label>Stock</label>
        <input type="number" min="0" name="stock" value="<?= h($p['stock']) ?>"></div>
      <div class="form-row"><label>Délai fabrication (jours)</label>
        <input type="number" min="0" name="delai_fabrication_jours" value="<?= h($p['delai_fabrication_jours'] ?? '') ?>"></div>
    </div>

    <div class="form-row"><label>Photo actuelle</label>
      <?php if (!empty($p['image_url'])): ?>
        <img src="<?= h(strpos($p['image_url'],'http')===0 ? $p['image_url'] : '../'.$p['image_url']) ?>"
             style="height:80px;border-radius:6px;object-fit:cover;margin-bottom:6px;display:block">
      <?php endif; ?>
      <input type="file" name="photo" accept="image/jpeg,image/png,image/webp">
      <span style="font-size:11px;color:var(--muted)">ou</span>
      <input type="url" name="image_url" placeholder="https://… URL externe" value="<?= h($_POST['image_url'] ?? '') ?>">
    </div>

    <div class="form-row" style="display:flex;gap:16px">
      <label><input type="checkbox" name="est_publie" value="1" <?= $p['est_publie']?'checked':'' ?> style="width:auto;margin-right:5px">Publié</label>
      <label><input type="checkbox" name="est_personnalisable" value="1" <?= $p['est_personnalisable']?'checked':'' ?> style="width:auto;margin-right:5px">Personnalisable</label>
    </div>

    <div class="form-actions">
      <button class="btn-primary" type="submit">Enregistrer</button>
      <a class="btn-ghost" href="produits.php">Annuler</a>
    </div>
  </form>
</div>
<?php require_once __DIR__ . '/_footer.php'; ?>
